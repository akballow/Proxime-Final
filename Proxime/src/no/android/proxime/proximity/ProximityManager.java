...
import no.android.proxime.profile.BleManager;
...
public class ProximityManager implements BleManager<ProximityManagerCallbacks> {
	...
	public int rssilevel = 10;
	public ProximityActivity act;
	...
	public ProximityManager(Context context) {
		...
		mHandler = new Handler(); //note this for later!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	private BluetoothGattServerCallback mGattServerCallbacks = new BluetoothGattServerCallback() {
		...
		public void onCharacteristicWriteRequest(BluetoothDevice device, int requestId, BluetoothGattCharacteristic characteristic, boolean preparedWrite, boolean responseNeeded, int offset,
			...
			if (receivedValue == HIGH_ALERT) {
				Logger.i(mLogSession, "[Proximity Server] Immediate alarm request received");
				playAlarm();
				getRssi();
				dropValue = 0;
			}else if (receivedValue == MID_ALERT) {
				Logger.i(mLogSession, "[Proximity Server] Immediate Warn request received");
				playNotification();
				mCallbacks.onDrop(); //--> test this
				dropValue = 1;
				playNotification();
			}  else {
				Logger.i(mLogSession, "[Proximity Server] Immediate off request received");
				stopAlarm();
				dropValue = 0;
			}
		}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		public void onReadRemoteRssi(BluetoothGatt gatt, int rssi, int status) {
			DebugLogger.d(TAG, "Start THE RSSI");
			if (rssi != 0) {
			rssilevel = rssi; }
			else {
			rssilevel = 0; }
			DebugLogger.d(TAG, "Finished THE RSSI");
		}
	...
	public void writeImmediateWarning() {
		if (mAlertLevelCharacteristic != null) {
			DebugLogger.d(TAG, "writing Immediate warning characteristic On");
			mAlertLevelCharacteristic.setWriteType(BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE);
			mAlertLevelCharacteristic.setValue(MID_ALERT, BluetoothGattCharacteristic.FORMAT_UINT8, 0);
			mBluetoothGatt.writeCharacteristic(mAlertLevelCharacteristic);
		} else {
			DebugLogger.w(TAG, "Immediate Warning Level Characteristic is not found");
		}
	}
